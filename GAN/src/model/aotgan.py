...
class AOTBlock(nn.Module):
    def __init__(self, dim, rates):
        ...
        nn.ReflectionPad2d((1, 1, 1, 1)),  # Explicit tuple for padding
        nn.Conv2d(dim, dim, kernel_size=3, padding=0, dilation=1)
    )
    ...
    def forward(self, x):
        ...
        out = self.fuse(out)
        #print(f"Shape after fuse: {out.shape}")
        mask = my_layer_norm(self.gate(x))
        #print(f"Shape after gate: {mask.shape}")
